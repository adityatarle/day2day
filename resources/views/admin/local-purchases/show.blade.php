...
@section('title', 'Local Purchase Approval')
...
<div class="container mx-auto px-4 py-8">
    <div class="flex justify-between items-center mb-6">
        <div>
            <div class="flex items-center text-sm text-gray-600 mb-2">
                <a href="{{ route('admin.local-purchases.index') }}" class="hover:text-gray-900">Local Purchase Approvals</a>
                <span class="mx-2">/</span>
                <span class="text-gray-900">{{ $localPurchase->purchase_number }}</span>
            </div>
            <h1 class="text-3xl font-bold text-gray-900">Local Purchase Approval</h1>
            <p class="text-gray-600">{{ $localPurchase->purchase_number }} - {{ $localPurchase->branch->name }}</p>
        </div>
        <div class="flex space-x-2">
            ...
            <button type="button" class="bg-green-600 hover:bg-green-700 text-white font-bold py-2 px-4 rounded-lg transition-colors" onclick="approvePurchase()">
                <i class="fas fa-check mr-2"></i>Approve Purchase
            </button>
            <button type="button" class="bg-red-600 hover:bg-red-700 text-white font-bold py-2 px-4 rounded-lg transition-colors" onclick="showRejectModal()">
                <i class="fas fa-times mr-2"></i>Reject Purchase
            </button>
            @endif
            
            <a href="{{ route('admin.local-purchases.index') }}" 
        ...
        <div class="flex">
            ...
            <div class="ml-3">
                ...
                <div class="mt-2 text-sm">
                    ...
                    <p><strong>Status:</strong> Stock updated and expense recorded</p>
                    @endif
                </div>
            </div>
        </div>
    </div>
    @elseif($localPurchase->isPending())
    <div class="bg-yellow-50 border border-yellow-200 text-yellow-700 px-4 py-3 rounded-lg mb-6">
        <div class="flex">
            ...
            <div class="ml-3">
                ...
                <div class="mt-2 text-sm">
                    <p>This local purchase is waiting for your approval. Review the details below and approve or reject the request.</p>
                    <p class="mt-1"><strong>Created:</strong> {{ $localPurchase->created_at->format('d M Y, h:i A') }} by {{ $localPurchase->manager->name }}</p>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-1 space-y-6">
            ...
            @if($localPurchase->isPending())
            <div class="bg-white rounded-lg shadow-sm border border-gray-200">
                <div class="bg-gradient-to-r from-amber-500 to-amber-600 text-white px-6 py-4 rounded-t-lg">
                    <h2 class="text-lg font-semibold">Approval Actions</h2>
                </div>
                <div class="p-6">
                    <div class="space-y-4">
                        <div class="text-center">
                            <p class="text-sm text-gray-600 mb-4">Review the purchase details and take action:</p>
                        </div>
                        
                        <button type="button" onclick="approvePurchase()" 
                        ...
                        <button type="button" onclick="showRejectModal()" 
                                class="w-full bg-red-600 hover:bg-red-700 text-white font-bold py-3 px-4 rounded-lg transition-colors flex items-center justify-center">
                            <i class="fas fa-times mr-2"></i>Reject Purchase
                        </button>
                    </div>
                    
                    <div class="mt-6 p-4 bg-blue-50 rounded-lg">
                        <h3 class="text-sm font-medium text-blue-900 mb-2">What happens when approved?</h3>
                        <ul class="text-xs text-blue-800 space-y-1">
                            <li>• Stock will be updated for the branch</li>
                            <li>• Expense record will be created</li>
                            <li>• Manager will be notified</li>
                            <li>• Purchase will be marked as completed</li>
                        </ul>
                    </div>
                </div>
            </div>
